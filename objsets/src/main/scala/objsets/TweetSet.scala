...
class Empty extends TweetSet {
  ...
  override def filter(p: Tweet => Boolean) = filterAcc(p, new Empty)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val tmpAcc = left.filter(p).union(right.filter(p))
    
    if (p(elem)) tmpAcc.incl(elem)
    else tmpAcc
  }
  
  override def filter(p: Tweet => Boolean) = filterAcc(p, new Empty)

  override def union(that: TweetSet) = that.union(left).union(right).incl(elem)
  
  override def mostRetweeted(): Tweet = {
    val leftList = left.descendingByRetweet
    val rightList = right.descendingByRetweet
    
    var max = elem
    
    if (!leftList.isEmpty && leftList.head.retweets > max.retweets) max = leftList.head
    if (!rightList.isEmpty && rightList.head.retweets > max.retweets) max = rightList.head
    max
  }
